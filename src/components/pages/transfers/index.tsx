 ...
 * @returns {JSX.Element} The rendered TransfersPage component.
 *
 * @example
...
export const TransfersPage = ({ query }: TransfersPageProps): JSX.Element => {
  const heads = ['Sender', 'Origin', 'Destiny', 'Receiver', 'Date', 'Status'];
  const title = 'Transfer';
  const addButton = <CreateTransferModal />;

  const { data, isFetching } = useFetch({
    promise: TransferServices.getAll(query),
